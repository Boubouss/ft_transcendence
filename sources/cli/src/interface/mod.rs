use crate::{size::Size, MyError, BACKGROUND_COLOR};
...
impl Interface {
    ...
    fn output_error(&mut self, error_msg: &MyError) {
        for element in &mut self.elements {
            if let Elements::Error(error) = element {
                match error_msg {
                    MyError::Mine(msg) => error.set_content(msg.clone()),
                    MyError::Serde(msg) => error.set_content(msg.to_string()),
                    MyError::Reqwest(msg) => error.set_content(msg.to_string()),
                };
            }
        }
    ...
    async fn handle_action(&mut self, input: char) -> Result<(), MyError> {
    ...
    pub async fn handle_input(&mut self, input: Key) -> Result<(), MyError> {
        match input {
