...
export function createAccountModalInner(): HTMLDivElement {
  const name = authStorage.getUserValue('name');
  ...
  inner.className = "relative bg-orange-500 text-black rounded-2xl p-6 h-[800px] w-[640px] border-2 border-black shadow-2xl";
  ...
  inner.innerHTML = `
    ...
    <div class="bg-[url('../../assets/images/main-menu_background.jpg')] absolute top-[120px] left-[460px] w-[160px] h-[160px] rounded-full border-2 border-black overflow-hidden">
      <img src="${avatarUrl}" alt="Avatar" class="w-full h-full object-cover" style="transform: scale(1.2);" />
    </div>
    <div class="absolute top-[280px] left-[235px] transform -translate-x-1/2 w-[430px] space-y-6">
      ...
      <div class="bg-[#FFFFFF99] rounded-[20px] border-2 border-black px-6 py-3 flex items-center justify-between">
        ...
        <input id="username" type="text" readonly value="${name}" required minlength="3" maxlength="20" class="bg-transparent text-black text-2xl font-jaro w-[250px] focus:outline-none" />
      </div>
      <div class="bg-[#FFFFFF99] rounded-[20px] border-2 border-black px-6 py-3 flex flex-col gap-3">
        <div class="flex items-center justify-between">
          <label for="password" class="text-black text-2xl font-jaro select-none">${pw} :</label>
