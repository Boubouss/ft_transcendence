...
export function createAccountPage(isModal = false): HTMLElement {
  ...
  container.innerHTML = `
    ...
    <div class="flex flex-col items-center gap-4 w-full">
      ...
      <div id="avatar-section" class="justify-center flex flex-col sm:flex-row items-center sm:items-start gap-4 w-full max-w-[400px]">
        ...
        <div id="side-buttons" class="hidden sm:flex flex-col gap-2"></div>
      </div>
      ...
      <div class="flex flex-col gap-4 w-full max-w-[400px]">
        ...
        <div class="bg-[#FFFFFF99] rounded-[20px] border-2 border-black px-4 py-3 flex items-center justify-between">
          ...
          <input id="username" type="text" readonly value="${username}" class="bg-transparent text-black text-base md:text-xl font-jaro text-right w-[60%] focus:outline-none" />
        </div>

        <div class="bg-[#FFFFFF99] rounded-[20px] border-2 border-black px-4 py-3 flex flex-col gap-3">
          <div class="flex items-center justify-between">
            <label for="password" class="text-black text-base md:text-xl font-jaro">${pw} :</label>
            <input id="password" type="password" readonly value="******" class="bg-transparent text-black text-base md:text-xl font-jaro w-[60%] text-right focus:outline-none" />
          </div>
          <div id="new-password-container" class="flex items-center justify-between hidden">
            <label for="new-password" class="text-black text-base md:text-xl font-jaro">${newpw} :</label>
            <input id="new-password" type="password" value="" class="bg-transparent text-black text-base md:text-xl font-jaro w-[60%] text-right focus:outline-none" />
          </div>
        </div>
      </div>

      <!-- Conteneur des boutons A2F et avatar pour mobile (à rendre visible dynamiquement) -->
      <div id="mobile-edit-buttons" class="flex sm:hidden flex-col gap-4 w-full max-w-[400px]" style="display: none;"></div>

      <!-- Conteneur principal des actions (edit/valider/annuler) -->
      <div id="button-container" class="flex flex-row justify-center items-center gap-4 w-full max-w-[400px] mt-8"></div>
    </div>
  `;
